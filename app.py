...
def main_app():
    ...
    daftar_barang = list(st.session_state["persediaan"].keys())
    if len(daftar_barang) == 0:
        st.warning("⚠️ Belum ada barang. Tambahkan barang dulu di sidebar.")
        st.stop()

    pilihan_barang = st.sidebar.selectbox("Pilih Barang", daftar_barang)
